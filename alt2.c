...
// Use of globals remove a tiny bit of overhead
int numberThreads;
double ** Ag;
double ** Bg;
double ** Cg;
...
void * team_matmul_thread(void * threadNo)
{
  int i, j, k;
  long int no = (long int)threadNo;
  int start = no * blockSize;
  int finish;
  if(no+1 == numberThreads)
  {
      finish = a_dim1g;
  }
  else
  {
      finish = (no + 1) * blockSize;
  }
...
void team_matmul(double ** A, double ** B, double ** C, int a_dim1, int a_dim2, int b_dim2)
{
    int t;
    long int rc;
    numberThreads = _SC_NPROCESSORS_ONLN;
    ...
    a_dim2g = a_dim2;
    b_dim2g = b_dim2;
    blockSize = a_dim1 / numberThreads;

    for (t = 0; t < numberThreads; t++)
    {
        rc = pthread_create(&threads[t], NULL , team_matmul_thread, (void *)t);
        if (rc)
        {
