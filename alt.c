...
void * team_matmul_thread(void * data)
{
  int i, j, k;
  struct data * temp = (struct data *)data;
  int start = temp->start;
  int finish = temp->finish;
  double ** A = temp->A;
  double ** B = temp->B;
  double ** C = temp->C;
  int a_dim2 = temp->a_dim2;
  int b_dim2 = temp->b_dim2;
...
void team_matmul(double ** A, double ** B, double ** C, int a_dim1, int a_dim2, int b_dim2)
{
    int t;
    long int rc;
    int num_threads = _SC_NPROCESSORS_ONLN;
    pthread_t threads[num_threads];
    struct data * data;
    int block_size = a_dim1 / num_threads;

    for (t = 0; t < num_threads-1; t++) // Create consumers
    {
        // TODO: REFACTOR: Move to struct init function
        data = malloc(sizeof(struct data));
        data->start = t * block_size;
        data->finish = (t+1) * block_size;
        data->A = A;
        data->B = B;
        data->C = C;
        data->a_dim2 = a_dim2;
        data->b_dim2 = b_dim2;
        rc = pthread_create(&threads[t], NULL , team_matmul_thread, (void *)data);
        if (rc)
        {
